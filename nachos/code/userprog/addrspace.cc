...
// first, set up the translation 
  ...
  KernelPageTable[i].shared = FALSE;
	// KernelPageTable[i].physicalPage = getNextPhysicalPage(i, FALSE);
    ...
    for (i = 0; i < numVirtualPages; i++) {
        ...
        if (parentPageTable[i].shared) {
          ...
          if (parentPageTable[i].valid) {
            KernelPageTable[i].physicalPage = getNextPhysicalPage(i, FALSE);
            //KernelPageTable[i].physicalPage = newAssigned + numPagesAllocated;
            newAssigned += 1;
          } else {
            KernelPageTable[i].physicalPage = 0;
          }
...
ProcessAddressSpace::addSharedMemory(unsigned int newMemory) {
    ...
    DEBUG('z', "Added %d Pages", numNewPages);
    ...
    for (i = 0; i < numNewPages; i++) {
      ...
      NewKernelPageTable[i+numVirtualPages].physicalPage = getNextPhysicalPage(i+numVirtualPages, FALSE);
...
unsigned
ProcessAddressSpace::getNextPhysicalPage(unsigned int virtualPage, bool replace) {
  stats->pageFaults += 1;
  if (replace) {
    // TODO COOL REPLACE STUFF
    return 0;
  } else {
    numPagesAllocated += 1;
...
void
ProcessAddressSpace::fixPageFault(unsigned int vadd, bool swap) {

  unsigned vpn = vadd / PageSize;
  unsigned physicalPage = getNextPhysicalPage(vpn, FALSE);

  OpenFile *executable = fileSystem->Open(fileName);

  KernelPageTable[vpn].physicalPage = physicalPage;
  KernelPageTable[vpn].valid = TRUE;

  unsigned copyStart = vpn * PageSize;
  if (swap) {
    // TODO GET FROM SWAP
  } else {
